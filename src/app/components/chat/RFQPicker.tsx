...
function toOpenRfqItem(row: Record<string, unknown>): OpenRfqItem | null {
  ...
  return {
    ...
    status,
    category_name:
      String(row.category_name ?? row.category ?? "").trim() || undefined,
    target_price:
...
export function RFQPicker({ conversationId, onSelect, onCancel }: Props) {
  ...
  const q = useQuery({
    ...
    queryFn: async () => {
      const [opRows, pdRows] = await Promise.all([
        rfqsApi.list("OP"),
        rfqsApi.list("PD"),
      ]);
      const op = (Array.isArray(opRows) ? opRows : []) as Record<
        string,
        unknown
      >[];
      const pd = (Array.isArray(pdRows) ? pdRows : []) as Record<
        string,
        unknown
      >[];
      const merged = [...op, ...pd];
    ...
    try {
      ...
      const message =
        ...
        typeof (res.data as Record<string, unknown>).message === "object"
          ? ((res.data as Record<string, unknown>).message as Record<
              string,
              unknown
            >)
          : undefined);
                  ...
                  <div className="flex items-start justify-between gap-2">
                    <div className="min-w-0">
                      ...
                      <p className="text-xs text-gray-500 mt-0.5">
                        {rfq.target_price
                          ? `งบ ${formatCurrency(rfq.target_price, "THB")}`
                          : "ไม่มีงบ"}
