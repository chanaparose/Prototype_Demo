...
export function FactoryHighlightField({
  ...
  return (
    <div>
      ...
      <div className={`rounded-xl border ${error ? 'border-red-300' : 'border-gray-200'} p-2.5 bg-white`}>
        <textarea
          value={value}
        ...
        <div className="mt-1 flex items-center justify-between">
          ...
          <span className={`text-[11px] tabular-nums ${counterClass}`}>{len}/{maxLength}</span>
        </div>
      </div>
      {isLocked ? (
