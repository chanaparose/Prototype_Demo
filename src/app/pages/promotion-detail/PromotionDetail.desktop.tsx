...
export function PromotionDetailDesktop() {
  ...
  const item = data.factoryShowcases.find((e) => e.id === id && e.contentType === 'promotion');
    ...
    return (
      <div className="hidden lg:block px-8 pt-8 pb-20 min-h-[calc(100vh-4rem)]" style={{ background: '#F8F6FA' }}>
        ...
        <div className="bg-white rounded-2xl border border-gray-100 p-10 text-center shadow-sm">
          ...
          <p className="text-[14px] text-gray-500 font-medium">ไม่พบข้อมูลโปรโมชัน</p>
        </div>
      </div>
    );
  }
        ...
        <div className="flex-1 min-w-0 space-y-5">
          ...
          <div className="bg-white rounded-2xl p-6 border border-gray-100 shadow-sm">
            ...
            <div className="space-y-4">
              {[
                {
                  icon: <CirclePercent className="w-5 h-5" style={{ color: '#7A4B94' }} />,
                  title: 'คำสั่งซื้อใหม่',
                  desc: 'สำหรับคำสั่งซื้อใหม่ที่เริ่มผลิตภายในช่วงแคมเปญเท่านั้น',
                },
                {
                  icon: <CalendarClock className="w-5 h-5" style={{ color: '#E38844' }} />,
                  title: 'ระยะเวลาผลิต',
                  desc: `ระยะเวลาผลิตโดยเฉลี่ย ${item.leadTime} ขึ้นอยู่กับสเปกและปริมาณจริง`,
                },
                {
                  icon: <TicketPercent className="w-5 h-5" style={{ color: '#7A4B94' }} />,
                  title: `MOQ ${item.minOrder}`,
                  desc: 'ขั้นต่ำการสั่งผลิตที่กำหนด สามารถเจรจาเพิ่มเติมได้ผ่านแชท',
                },
              ].map((s, i) => (
