        ...
        <div>
          ...
          <div className="grid grid-cols-4 gap-1">
            {Array.from({ length: 4 }).map((_, i) => (
              <div key={i} className={`h-1.5 rounded ${i < strength ? 'bg-emerald-500' : 'bg-slate-200'}`} />
            ))}
          </div>
        </div>

        <button type="button" onClick={() => void submit()} disabled={saving} className="w-full py-2.5 rounded-xl bg-indigo-600 text-white text-sm font-semibold disabled:opacity-50">
