...
export function EditProfilePage() {
  ...
  useEffect(() => {
    ...
    }).catch((e) => setError(e instanceof Error ? e.message : 'โหลดข้อมูลไม่สำเร็จ')).finally(() => mounted && setLoading(false));
    return () => {
      mounted = false;
    };
  }, []);
  ...
  return (
    <div className="space-y-4 pb-24">
      <div className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm flex items-center justify-between">
        ...
        <Button variant="unstyled" type="button" onClick={() => void onSave()} disabled={saving} className="text-xs font-semibold text-indigo-600 disabled:opacity-50">บันทึก</Button>
      </div>
      ...
      <div className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm space-y-3">
        <div className="flex items-center gap-3">
          <img src={avatarPreview || '/assets/avatars/customer-default.svg'} alt="avatar" className="w-14 h-14 rounded-2xl object-cover bg-slate-100" />
