  ...
  return (
    <div className="max-w-5xl mx-auto px-4 py-6 space-y-4">
      ...
      <div className="bg-white rounded-2xl border border-gray-100 p-4 space-y-1">
        ...
        <div className="flex justify-between text-sm"><span>Subtotal</span><MoneyText value={Number(breakdown.subtotal ?? 0)} currencyCode={currency} /></div>
        <div className="flex justify-between text-sm"><span>Shipping</span><MoneyText value={Number(breakdown.shipping_cost ?? 0)} currencyCode={currency} /></div>
        <div className="flex justify-between text-sm"><span>Packaging</span><MoneyText value={Number(breakdown.packaging_cost ?? 0)} currencyCode={currency} /></div>
        <div className="flex justify-between text-sm"><span>Tooling</span><MoneyText value={Number(breakdown.tooling_mold_cost ?? 0)} currencyCode={currency} /></div>
        <div className="flex justify-between text-sm"><span>VAT</span><MoneyText value={Number(breakdown.vat_amount ?? 0)} currencyCode={currency} /></div>
        <hr />
        <div className="flex justify-between font-semibold"><span>Grand Total</span><MoneyText value={Number(breakdown.grand_total ?? 0)} currencyCode={currency} /></div>
      </div>
        ...
        <div className="bg-white rounded-2xl border border-gray-100 p-4 space-y-2">
          ...
          {compare ? (
            <div className="rounded-xl bg-gray-50 p-2">
              <DiffRow label="Subtotal" before={String(compare.before.subtotal ?? '-')} after={String(compare.after.subtotal ?? '-')} changed={compare.before.subtotal !== compare.after.subtotal} />
              <DiffRow label="Lead time" before={String(compare.before.lead_time_days ?? '-')} after={String(compare.after.lead_time_days ?? '-')} changed={compare.before.lead_time_days !== compare.after.lead_time_days} />
              <DiffRow label="Grand total" before={String(compare.before.grand_total ?? '-')} after={String(compare.after.grand_total ?? '-')} changed={compare.before.grand_total !== compare.after.grand_total} />
            </div>
          ) : null}
        </div>
      ) : null}
