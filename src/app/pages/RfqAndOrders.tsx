...
export function RfqAndOrders() {
  ...
  // Desktop RFQ filter
  const [desktopRfqFilter, setDesktopRfqFilter] = useState<RfqFilterId>('pending');
  const [desktopOrderFilter, setDesktopOrderFilter] = useState<OrderFilterId>('in_production');

  const desktopFilteredRfqs = useMemo(() => {
    return rfqs.filter((r) => {
      if (r.status === 'completed') return false;
      if (desktopRfqFilter === 'cancelled_expired')
        return r.status === 'cancelled' || r.status === 'expired';
      const fid = getRfqFilterId(r.status);
      return fid === desktopRfqFilter;
    });
  }, [desktopRfqFilter]);

  const desktopFilteredOrders = orders.filter((o) => o.status === desktopOrderFilter);
    ...
    <>
      ...
      <div className="lg:hidden pb-4 flex flex-col min-h-full pb-20">
        ...
        <div className="px-4 flex-1 flex flex-col">
          {/* Primary Tabs */}
          <div className="flex p-1 rounded-2xl mb-4" style={{ background: PRIMARY_BG }}>
            <button
      ...
      <div className="hidden lg:block px-8 py-7">
        ...
        <div className="flex items-center justify-between mb-6">
          <div>
            ...
            <h1 className="text-2xl font-bold text-gray-900">RFQ & คำสั่งซื้อ</h1>
          </div>
          <Link
            to="/create-rfq"
        ...
        <div className="grid grid-cols-4 gap-4 mb-6">
          {[
            {
              label: 'RFQ รอดำเนินการ',
              value: rfqTagCounts.pending,
              color: '#EA580C',
              bg: '#FFEDD5',
              icon: Clock3,
            },
            ...
            {
              label: 'กำลังผลิต',
              value: orderTagCounts.inProduction,
              color: '#3B82F6',
              bg: '#DBEAFE',
              icon: Package,
            },
        ...
        <div className="grid grid-cols-2 gap-6">
          ...
          <div className="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
            ...
            <div className="flex gap-1 px-4 pt-3 pb-2">
              {[
                { id: 'pending' as RfqFilterId, label: 'รอดำเนินการ', icon: Clock3, count: rfqTagCounts.pending },
                { id: 'has_quote' as RfqFilterId, label: 'มีใบเสนอราคา', icon: FileCheck2, count: rfqTagCounts.has_quote },
                { id: 'cancelled_expired' as RfqFilterId, label: 'ยกเลิก/หมดอายุ', icon: OctagonX, count: rfqTagCounts.cancelled_expired },
              ].map((tab) => (
                <button
                  key={tab.id}
                  onClick={() => setDesktopRfqFilter(tab.id)}
                        ...
                        <div className="flex items-start justify-between gap-2 mb-2">
                          <div className="flex gap-2.5 min-w-0 flex-1">
                            <div
                              ...
                              style={{ background: PRIMARY_BG }}
                            >
                              {rfq.categoryIcon ?? <Layers size={18} style={{ color: PRIMARY_COLOR }} />}
                            </div>
          ...
          <div className="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
            ...
            <div className="flex gap-1 px-4 pt-3 pb-2">
              {[
                { id: 'in_production' as OrderFilterId, label: 'กำลังผลิต', icon: Package, count: orderTagCounts.inProduction, color: '#3B82F6', bg: '#DBEAFE' },
                { id: 'shipped' as OrderFilterId, label: 'จัดส่งแล้ว', icon: Truck, count: orderTagCounts.shipped, color: '#F59E0B', bg: '#FEF3C7' },
                { id: 'completed' as OrderFilterId, label: 'เสร็จสิ้น', icon: CheckCircle2, count: orderTagCounts.completed, color: '#22C55E', bg: '#DCFCE7' },
              ].map((tab) => {
                const isActive = desktopOrderFilter === tab.id;
                return (
                  <button
                    key={tab.id}
                    onClick={() => setDesktopOrderFilter(tab.id)}
                    className="flex items-center gap-1.5 px-3 py-1.5 rounded-xl text-xs transition-all"
                    style={{
