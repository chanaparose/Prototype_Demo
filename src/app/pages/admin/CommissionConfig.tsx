...
export function CommissionConfig() {
  ...
  React.useEffect(() => {
    void (async () => {
      const [a, h] = await Promise.all([platformConfigApi.getActive(), platformConfigApi.history()]);
      setActive(a);
      setHistory(h);
      ...
      <div className="bg-white rounded-2xl border border-gray-100 p-4 space-y-3">
        ...
        <div className="border-t border-gray-100 pt-3 space-y-3">
          <label className="inline-flex items-center gap-2 text-sm">
            <input type="checkbox" checked={form.promo_enabled} onChange={(e) => setForm({ ...form, promo_enabled: e.target.checked })} />
            Enable Promo
          </label>
          {form.promo_enabled ? (
            <>
        ...
        <button
          ...
          disabled={saving}
          onClick={async () => {
            const ok = window.confirm('การเปลี่ยน rate จะมีผลกับ quote ใหม่เท่านั้น quote ที่ submit แล้วใช้ rate เดิม');
            if (!ok) return;
            setSaving(true);
            try {
